...
public class Game {
    ...
    public String checkGameWinner(char [][]grid){
        ...
        //Indicator of Tie scenario
        boolean indTie = false;
        for (int i = 0; i < 3; i++){
            for (int j = 0; j < 3; j++) {
                char base = grid[i][j];
                if (base != '-') {
                    if (i == j) {
                        if (base == grid[i][(j + 1) % 3] && base == grid[i][(j + 2) % 3] ||
                                base == grid[(i + 1) % 3][j] && base == grid[(i + 2) % 3][j] ||
                                base == grid[(i + 1) % 3][(j + 1) % 3] && base == grid[(i + 2) % 3][(j + 2) % 3]) {
                            switch (base) {
                                case 'x':
                                    return "X wins";
                                case 'o':
